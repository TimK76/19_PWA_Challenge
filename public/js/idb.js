...
// This function will be executed if we attmpt to submit a new transaction and there's no internet connection
...
function sendTransaction() {
  ...
  getAll.onsuccess = function () {
    // if ther was data in indexedDB's store, let's send it to the api server
    if (getAll.result.length > 0) {
      fetch("/api/transaction/bulk", {
        ...
        .then((serverResponse) => {
          ...
          const transaction = db.transaction(["new_budget"], "readwrite");
          // access the new_budgetobject store
          const budgetObjectStore = transaction.objectStore("new_budget");
